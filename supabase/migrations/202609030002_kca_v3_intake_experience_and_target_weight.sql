-- GodHealth KCA v3 intake experience repair.
-- Updates the live V3 assessment definition and makes target body weight part of the safe plan estimate.

create or replace function public.kca_v3_apply_intake_experience_updates()
returns void
language plpgsql
security definer
...
begin
  with fields as (
    select elem, ord
    from public.kca_assessment_definitions d,
      jsonb_array_elements(d.definition->'personal_transformation_intake') with ordinality as x(elem, ord)
    where d.definition_version = '3.0.0'
      and d.retired_at is null
      and elem->>'id' not in ('GL3', 'PR10')
  ),
  adjusted as (
    select
      (ord * 10)::numeric as sort_key,
      case
        when elem->>'id' = 'PR2' then
          (elem - 'show_if')
          || jsonb_build_object(
            'label', 'Gender',
            'type', 'single_select',
            'required', true,
            'options', jsonb_build_array('man', 'woman')
          )
        when elem->>'id' = 'PR9' then
          (elem - 'show_if')
          || jsonb_build_object(
            'label', 'Typical weekly schedule constraints',
            'type', 'multi_select',
            'required', true,
            'options', jsonb_build_array(
              'early mornings are difficult',
              'late evenings are difficult',
              ...
              'weekends disrupt routine',
              'other'
            )
          )
        when elem->>'id' = 'GL5' then
          (elem - 'show_if')
          || jsonb_build_object(
            'label', 'Target body weight (kg) — the weight you want to move toward',
            'type', 'decimal',
            'required', false
          )
        else elem
    ...
    select
      ((select ord from fields where elem->>'id' = 'PR9' limit 1) * 10 + 1)::numeric as sort_key,
      jsonb_build_object(
        'id', 'PR10',
        'section', 'profile',
  ...
  select jsonb_agg(elem order by sort_key) into v_new_intake
  from adjusted
  where sort_key is not null;

  if v_new_intake is not null then
    ...
    change_reason = 'V3 intake UX repair: gender options, schedule choices, work-hours field, target-weight clarity, removed duplicate 12-week success prompt.'
...
select public.kca_v3_apply_intake_experience_updates();

drop function public.kca_v3_apply_intake_experience_updates();
...
declare
  ...
  v_gender text := lower(coalesce(p_intake->>'PR2',''));
  ...
  v_target_weight numeric := public.kca_v3_safe_numeric(p_intake->>'GL5', null);
  v_target_direction text := null;
...
begin
  if v_target_weight is not null and v_weight is not null and v_target_weight > 0 and v_weight > 0 then
    if v_target_weight < v_weight - 1 then
      v_target_direction := 'fat_loss';
    elsif v_target_weight > v_weight + 1 then
      v_target_direction := 'weight_gain';
    else
      v_target_direction := 'maintenance';
    end if;
  end if;

  if v_gender in ('male', 'man') and v_age is not null and v_height is not null and v_weight is not null then
    v_ree := round(10*v_weight + 6.25*v_height - 5*v_age + 5, 2);
  elsif v_gender in ('female', 'woman') and v_age is not null and v_height is not null and v_weight is not null then
  ...
  if v_ree is not null then
    ...
    if jsonb_array_length(v_hard_blocks) = 0 then
      if lower(v_goal) = 'fat loss' or v_target_direction = 'fat_loss' then
        v_goal_range := jsonb_build_object('low', round(v_tdee * .80), 'high', round(v_tdee * .90), 'approval_required', true);
      elsif lower(v_goal) = 'strength/muscle gain' or v_target_direction = 'weight_gain' then
  ...
  return jsonb_build_object(
    ...
    'summary', jsonb_build_object('primary_goal', v_goal, 'target_weight_kg', v_target_weight, 'calling_why', p_intake->>'GL4', 'constraints', p_intake->'GL8', 'safety_status', case when coalesce((p_safety->>'has_flags')::boolean,false) then 'coach/clinical review required' else 'ready for coach review' end),
    'energy_profile', jsonb_build_object(
      ...
      'target_weight_direction', v_target_direction,
      'weight_change_context_kg', case when v_target_weight is null or v_weight is null then null else round(v_target_weight - v_weight, 2) end,
    ...
    'recovery', jsonb_build_object('sleep_target','Adults generally need a 7-9 hour sleep opportunity.', 'apnea_like_symptoms_flag', lower(coalesce(p_intake->>'SL5','no')) in ('yes','not sure'), 'hydration_benchmark', case when v_gender in ('female','woman') then 'EFSA benchmark: about 2.0 L/day total water from food + beverages.' when v_gender in ('male','man') then 'EFSA benchmark: about 2.5 L/day total water from food + beverages.' else 'Use total water from food + beverages.' end, 'sweat_rate_formula','(pre_kg - post_kg + fluid_L - urine_L) / hours'),
