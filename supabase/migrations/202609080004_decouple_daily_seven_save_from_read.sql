-- GodHealth KCA — Decouple Daily Seven save from coach readback
-- Root fix: a write RPC must not roll back saved standards because the
-- follow-up dashboard read fails. The frontend already reloads the dashboard
-- after a successful save, so this function now returns a compact save receipt.
