...
begin
  ...
  with intake_labels(id, label) as (
    values
      ('PR1', 'How old are you?'),
      ('PR2', 'Gender'),
  ...
  select jsonb_agg(
    case
      when intake_labels.label is not null
        then jsonb_set(field, '{label}', to_jsonb(intake_labels.label), true)
      else field
    end
    order by ord
  )
  ...
  left join intake_labels on intake_labels.id = field->>'id';

  with core_statements(id, statement) as (
    values
  ...
  select jsonb_agg(
    case
      when core_statements.statement is not null
        then jsonb_set(question, '{statement}', to_jsonb(core_statements.statement), true)
      else question
    end
    order by ord
  )
  ...
  left join core_statements on core_statements.id = question->>'id';
