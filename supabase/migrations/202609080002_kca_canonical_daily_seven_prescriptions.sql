-- GodHealth KCA — Canonical Daily Seven prescription selection
-- Fixes stale Daily Seven display when older active prescriptions still exist.
-- The client app and coach dashboard now always read the newest valid
-- prescription per foundation/day.
  ...
  with ranked_prescriptions as (
    select
      ...
      row_number() over (
        ...
        order by
          case
            when current_date >= p.active_from and (p.active_until is null or current_date <= p.active_until) then 0
            else 1
          end,
          p.active_from desc,
          p.updated_at desc,
